...
orderManagement.unprocessedOrders = function() {
	jQuery("#list10").jqGrid(
					{
						...
						colModel : [ {name : 'orderNumber',index : 'orderNumber',width : 120,align : "center",sorttype : "int",key : true},
						             {name : 'loadNumber',index : 'loadNumber',width : 100,align : "center",sorttype : "int"},
						             {name : 'latestTime',index : 'latestTime',width : 160,align : "center",align : "center"},
						             {name : 'stopSeq',index : 'stopSeq',width : 80,align : "center",sorttype : "int"},
						             {name : 'customerNumber',index : 'customerNumber',width : 60,align : "center",sorttype : "int"}, 
						             {name : 'orderStatus',index : 'orderStatus',width : 150,align : "center"} ],
								...
								if (jQuery("#list10_d").jqGrid('getGridParam',
									...
									jQuery("#list10_d")
											.jqGrid(
													'setGridParam',
													{
														url : "/cpsb/unprocessedOrders.do?method=getUnprocessedOrderDetails&orderNum="+ ids,
														datatype : "json"
													}).trigger('reloadGrid');
								}
							} else {
	...
	jQuery("#list10").jqGrid('navGrid', '#pager10', {excel : true,view : true,add : false,edit : false,del : false,search : true,searchtext : "Filter"
	}, {}, {}, {}, {multipleSearch : true});
	 ...
	 * jQuery("#list10").excelExport("export.xml"); } });
	 */

	jQuery("#selectAll").click(function() {
		$("#list10").jqGrid('resetSelection');
		var ids = $("#list10").jqGrid('getDataIDs');
		for ( var i = 0, il = ids.length; i < il; i++) {
			$("#list10").jqGrid('setSelection', ids[i], true);
		}
	...
	jQuery("#wics").click(function() {
						...
						$.ajax( {
									type : "POST",
									url : "/cpsb/unprocessedOrders.do?method=throwBackToWics&orderNumbers="+ ids,
	...
	jQuery("#list10_d").jqGrid(
			{
				...
				colModel : [ {name : 'orderNumber',index : 'orderNumber',width : 55,align : "center"},
				             {name : 'sku',index : 'sku',width : 55,align : "center"},
				             {name : 'upc',index : 'upc',width : 80,align : "center"}, 
				             {name : 'itemDescription',index : 'itemDescription',width : 150,align : "center"},
				             {name : 'quantityOrdered',index : 'quantityOrdered',width : 150,align : "center",search : true},
				             {name : 'teachInId',index : 'teachInId',width : 150,align : "center",editable : true,edittype : "checkbox",formatter : 'checkbox',editoptions : {value : "true:false"}
				} ],
				rowNum : 10,
				rowList : [ 10, 20, 30 ],
				jsonReader : {
...
//shift and control key for block selection and multiple selection respectively
				beforeSelectRow : function multiSelectHandler(sid, e) {
					var grid = $(e.target).closest("table.ui-jqgrid-btable");
					var ts = grid[0], td = e.target;
					var scb = $(td).hasClass("cbox");
				...
				height : 230
			}).navGrid('#pager10_d', {view : true,add : false,edit : false,del : false,searchtext : "Filter"}, {}, {}, {}, {multipleSearch : true});
	$("#list10_d").jqGrid('hideCol', 'cb');

	$("#selectAll").button( {icons : {primary : 'ui-icon-circle-check'}});
	$("#clear").button( {icons : {primary : 'ui-icon-minusthick'}});
	$("#wics").button( {icons : {primary : 'ui-icon-arrowreturnthick-1-w'}});
	$("#relCasePick").button( {icons : {primary : 'ui-icon-gear'}});
	$("#teachhold").button( {icons : {primary : 'ui-icon-gear'}});
...
orderManagement.releasedOrders = function() {
	...
	jQuery("#list10")
			.jqGrid(
					{
						sortable : true,
						url : '/cpsb/processedOrders.do?method=getInitialprocessedList',
						datatype : 'json',
						colNames : [ 'Order', 'Load', 'Gate Time', 'Stop',
								'CP Cases', 'CP Pallets', 'Pallets Complete',
								'Customer', 'Status', 'Completion Date' ],
						colModel : [ {name : 'orderId',index : 'orderId',width : 85,sorttype : "int",align : "center",key : true},
						             {name : 'loadNumber',index : 'loadNumber',width : 40,sorttype : "int",align : "center"},
						             {name : 'latestTime',index : ' latestTime',width : 140,align : "center"},
						             {name : 'stopSeq',index : 'stopSeq',width : 40,align : "center",sorttype : "int"},
						             {name : 'cpCases',index : 'cpCases',width : 60,align : "center",sorttype : "int"},
						             {name : 'cpPallets',index : 'cpPallets',width : 60,align : "center",sorttype : "int"},
						             {name : 'palletsComplete',index : 'palletsComplete',width : 120,align : "center",sorttype : "int"},
						             {name : 'customer',index : 'customer',width : 60,align : "center",sorttype : "int"},
						             {name : 'status',index : 'status',width : 120,align : "center"},
						             {name : 'completionDate',index : 'completionDate',width : 150,align : "center"} ],
						...
						onSelectRow : function(ids) {
							if (ids == null) {
								...
								{
									jQuery("#list10_d").jqGrid('setCaption',"Order Header: " + ids);
									jQuery("#list10_d")
											.jqGrid(
													'setGridParam',
													{
														url : "/cpsb/processedOrders.do?method=getInitialprocessedListDetails&orderNum="+ ids,
														datatype : "json"
													}).trigger('reloadGrid');
								}
							} else {
								jQuery("#list10_d").jqGrid('setCaption',"Order Details: " + ids);
								jQuery("#list10_d").jqGrid('clearGridData');
								jQuery("#list10_d")
										.jqGrid('setGridParam',{url : "/cpsb/processedOrders.do?method=getInitialprocessedListDetails&orderNum="+ ids,datatype : "json"
												}).trigger('reloadGrid');
	...
	jQuery("#pselectAll").click(function() {
		$("#list10").jqGrid('resetSelection');
		var ids = $("#list10").jqGrid('getDataIDs');
		for ( var i = 0, il = ids.length; i < il; i++) {
			$("#list10").jqGrid('setSelection', ids[i], true);
	...
	jQuery("#pcancel")
			.click(function() {
				...
				if (ids.length > 0) {
					var names = [];
					for ( var i = 0, il = ids.length; i < il; i++) {
						var name = grid.jqGrid('getCell', ids[i], 'orderId');
						names.push(name);
					}
					// alert ("Names: " + names.join(", ") + "; ids:
					// " + ids.join(", "));
						...
						.dialog(
								{
									...
									buttons : {
										'Confirm' : function() {
											...
											$.ajax( {
														type : "POST",
														url : "/cpsb/processedOrders.do?method=cancelOrder&orderNumbers="+ ids,
														data : JSON.stringify(ids),
														dataType : "json"
													});
											$(this).dialog('close');
	...
	jQuery("#pcancelwics").click(function() {
				var grid = $("#list10");
				var ids = grid.jqGrid('getGridParam', 'selarrrow');
				if (ids.length > 0) {
					var names = [];
								...
								{resizable : true,
									...
									buttons : {
										'Confirm' : function() {
											...
											$.ajax( {
														type : "POST",
														url : "/cpsb/processedOrders.do?method=cancelAndReleaseToWics&orderNumbers="+ ids,
														data : JSON.stringify(ids),
														dataType : "json"
													});
											$(this).dialog('close');
	...
	jQuery("#list10_d").jqGrid(
			{
				sortable : true,
				colNames : [ 'Order', 'LPN', 'SKU', 'UPC', 'Item Description',
						'Quantity Ordered', 'Quantity Fulfilled',
						'Quantity Scratched' ],
				colModel : [ {name : 'orderNumber',index : 'orderNumber',width : 90,align : "center"}, 
				             {name : 'lpn',index : 'lpn',width : 55,align : "center"},
				             {name : 'sku',index : 'sku',width : 55,align : "center"},
				             {name : 'upc',index : 'upc',width : 80,align : "center"},
				             {name : 'itemDescription',index : 'itemDescription',width : 150,align : "center"},
				             {name : 'quantity',index : 'quantity',width : 150,align : "center",sortable : false,search : false},
				             {name : 'quantityFulfilled',index : 'quantityFulfilled',width : 120,align : "center"},
				             {name : 'quantityScratched',index : 'quantityScratched',width : 120,align : "center"} ],
				...
				caption : "Order Detail",
				height : 230
			}).navGrid('#pager10_d', {view : true,add : false,edit : false,del : false}, {}, {}, {}, {multipleSearch : true});
	$("#list10_d").jqGrid('hideCol', 'cb');

	$("#pselectAll").button( {icons : {primary : 'ui-icon-circle-check'}});
	$("#pclear").button( {icons : {primary : 'ui-icon-minusthick'}});
	$("#pcancel").button( {icons : {primary : 'ui-icon-circle-close'}});
	$("#pcancelwics").button( {icons : {primary : 'ui-icon-gear'}});
};
...
orderManagement.internalOrders = function() {
  ...
  jQuery("#e").datetimepicker({
	...
	jQuery("#internalOrder").jqGrid( {

		sortable : true,
		emptyrecords : "No records found",
		colNames : [ 'LPN', 'SKU','Description', 'Cases', 'Hold?', 'Expiration Date' ],
		colModel : [ {
			name : 'licensePlateNumber',index : 'licensePlateNumber',width : 160,sorttype : "int",align : "center"},
			{name : 'sku',index : 'sku',width : 120,sorttype : "int",align : "center"},
			{name : 'description',index : 'description',width : 150,align : "center",sorttype : "text"},
			{name : 'caseQuantity',index : 'caseQuantity',width : 130,align : "center",sorttype : "int"},
			{name : 'isHeld',index : 'isHeld',width : 160,align : "center"},
			{name : 'expirationDate',index : 'expirationDate',width : 140,align : "center"} ],
	...
	jQuery("#internalOrder").jqGrid('navGrid', '#pager', {view : true,add : false,edit : false,del : false,searchtext : 'Filter'}, {}, {}, {}, {multipleSearch : true});
	jQuery("#internalOrder").jqGrid('hideCol', 'cb');

	jQuery("#orderPreview").jqGrid( {
		sortable : true,
		emptyrecords : "No records found",
		colNames : [ 'LPN', 'SKU','Description', 'Cases', 'Hold?', 'Expiration Date' ],
		colModel : [ {name : 'licensePlateNumber',index : 'licensePlateNumber',width : 160,sorttype : "int",align : "center"},
			{name : 'sku',index : 'sku',width : 120,sorttype : "int",align : "center"},
			{name : 'description',index : 'description',width : 150,align : "center",sorttype : "text"},
			{name : 'caseQuantity',index : 'caseQuantity',width : 130,align : "center",sorttype : "int"},
			{name : 'isHeld',index : 'isHeld',width : 160,align : "center"},
		...
		viewrecords : true,
		loadonce : true,
		caption : "Order Preview",
		height : "100%"
			
	});
	jQuery("#orderPreview").jqGrid('navGrid', '#pager2', {add:false,edit:false,del:false,refresh:false,search:false});
	jQuery("#orderPreview").jqGrid('hideCol', 'cb');

	$("form#internalOrderForm").submit(
					function() {
						var newUrl = "/cpsb/internalOrder.do?method=showList&"+ $(this).serialize();
						$("#internalOrder").jqGrid("setGridParam", {"url" : newUrl,	datatype : "json"}).trigger("reloadGrid");
						return false;
	});

	$(".addorder").button({icons:{primary:'ui-icon-circle-plus'}}).click(function() {
		var mainOrders = $("#internalOrder");
		var orderPreview = $("#orderPreview");
		var previewIds = mainOrders.jqGrid('getGridParam', 'selarrrow');
		for ( var iProd = 0, lProd = previewIds.length; iProd < lProd; iProd++) {
			var prodId = previewIds[iProd];
			var prodData = mainOrders.jqGrid('getRowData', prodId);
			orderPreview.jqGrid('addRowData', prodId, prodData);
		}
	});
	
	$("#orderPreview ").find("tr:eq(0)").remove();
//TODO Final submit internal order
	$(".sorder").button({icons:{primary:'ui-icon-circle-arrow-e'}}).click(function(){
		var earliestDate = jQuery("input#e").val();
        var latestTimeDate =jQuery("input#l").val();
	    var gridData = jQuery("#orderPreview").getRowData();
	    
	    
	    $.ajax({
	        type: "POST",
	        url: "/cpsb/internalOrderList.do?method=create",
	        data : {
	            orders:JSON.stringify(gridData),
	        ...
	        success:function(message) {
	        	...
	        	if (message === 'Add Order messages sent for Internal Orders') {
					...
					$("#dialog-message").find('.success').hide().end().find('.error').show().find('.message').text(message).end().end().dialog( {
								resizable : false,
								height : 180,
								modal : true,
								buttons : {
			...
			complete: function(){
				$("#orderPreview").find("tr").remove();
				$("#internalOrder").find("tr").remove();
				jQuery("#e").val('');
				jQuery("#l").val('');
				jQuery("#lpn").val('');
				jQuery("#sku").val('');
	...
	jQuery(".sa").button( {icons : {primary : 'ui-icon-circle-check'}}).click(function() {
		$("#internalOrder").jqGrid('resetSelection');
		var ids = $("#internalOrder").jqGrid('getDataIDs');
		for ( var i = 0, il = ids.length; i < il; i++) {
			$("#internalOrder").jqGrid('setSelection', ids[i], true);
		}
	...
	$(".removeselected").button( {icons : {primary : 'ui-icon-trash'}}).click(function(){
	 var $table=$("#orderPreview");
...
errorControlLog.businessErrors = function() {
	jQuery("#datepicker").datepicker( {
		...
		jQuery("#businessError").jqGrid('setGridParam', {
			url : "/cpsb/businessErrors.do?method=getBusinessErrors",
			postData : {
	...
	jQuery("#businessError").jqGrid(
			{
				sortable : true,
				datatype : 'json',
				colNames : [ 'Error Number', 'Error Message', 'Created date',
						'Created User', 'Last Modified Date',
						'Last Modified User' ],
				colModel : [ {
					name : 'errorNumber',
					index : 'errorNumber',
...
items.addSpog = function() {
	$.validator.setDefaults( {
		submitHandler : function() {
			...
			$.ajax( {
				url : "/cpsb/spogMaster.do",
				type : "POST",
				data : {
					method : "findAndAddSpog",
					spogName : spogName,
				...
				success : function(message) {
					...
					if (message === 'Spog Added') {
						$("#dialog-message").find('.error').hide().end().find(
								'.success').show().end().dialog( {
							resizable : false,
							height : 180,
							modal : true,
							buttons : {
						...
						$("#dialog-message").find('.success').hide().end()
								.find('.error').show().find('.message').text(
										message).end().end().dialog( {
									resizable : false,
									height : 180,
									modal : true,
									buttons : {
...
inventoryView.inventoryView = function() {
	...
	jQuery("#inventoryInq").jqGrid(
			{
				sortable : true,
				emptyrecords : "No records found",
				colNames : [ 'LPN', 'SKU', 'Location Description',
						'Location Type', 'Pallet Status', 'On Hand Quantity',
						'Inducted Quantity', 'Rejected Qty', 'Hold?',
						'Expiration Date' ],
				colModel : [ {name : 'licensePlateNumber',index : 'licensePlateNumber',width : 140,sorttype : "int",align : "center"},
				             ...
				             {name : 'palletStatus',index : 'palletStatus',width : 80,align : "center",sorttype : "text"},
				             {name : 'caseQuantity',index : 'caseQuantity',width : 130,align : "center",sorttype : "int"},
				             ...
				             {name : 'isHeld',index : 'isHeld',width : 60,align : "center"},
	...
	jQuery("#inventoryInq").jqGrid('navGrid', '#pager', {
		excel : true,
		view : true,
		add : false,
		edit : false,
	...
	jQuery("#inventoryInq").jqGrid('navButtonAdd', '#pager', {
		caption : "Export to Excel",
		onClickButton : function() {
			jQuery("#inventoryInq").excelExport();
		}
	});
	$("form#inventoryForm").submit(
					function() {
						var newUrl = "/cpsb/inventoryInquiry.do?method=getInventoryDetails&"+ $(this).serialize();
						$("#inventoryInq").jqGrid("setGridParam", {"url" : newUrl,datatype : "json"}).trigger("reloadGrid");
						return false;
					});

	jQuery("#viewform").submit(function() {
...
items.notTaught = function() {
	...
	jQuery("#notTaught").jqGrid(
					{
						sortable : true,
						url : '/cpsb/jsp/notTaughtInItems/notTaughtInItemsWithCPSPrimaryorSecondary.do?method=getNotTaughtItemWithCPSPrimaryOrSecondary',
						datatype : 'json',
						colNames : [ 'SKU', 'Items Description' ],
						colModel : [ {name : 'sku',index : 'sku',width : 300,sorttype : "int",align : "center",key : true},
						             {name : 'itemDescription',index : 'itemDescription',width : 300,sorttype : "text",align : "center"} ],
						...
						gridview : true,
						autowidth:true,
						caption : "Not Taught In Items",
						height : 300
					});
	jQuery("#notTaught").jqGrid('navGrid', {
		view : true,
		add : false,
		edit : false,
		del : false
...
transactionHistory.transactionHistory = function(){
  ...
  jQuery("#edate").datepicker({
		disabled:true,
	  	showOn:'button',
		buttonImage:'../../image/icon_cal2.png',
		buttonText:"end date",
		buttonImageOnly:true,
		changeMonth: true,
	    changeYear: true
	  });
  $("img[src$='icon_cal2.png']").hide(); 
  jQuery("#transactionHistory").jqGrid({
        	  		sortable:true,
        	     colNames:['LPN','SKU', 'Transaction Type', 'From Status','To Status','From Quantity', 'To Quantity','Qty Type Modified','From Location','To Location','Reason','From Expiration date','To Expiration Date','Modified Date' ], 
        	     colModel:[  {name:'licensePlateNumber',index:'licensePlateNumber', width:85, sorttype:"int", align:"center", key:true},
        	        	     {name:'sku',index:'sku', width:40, sorttype:"int", align:"center"},
        	        	     {name:'transactionNumber',index:'transactionNumber', width:130, align:"center"},
        	        	     {name:'fromStatus',index:'fromStatus', width:100, align:"center"}, 
        	        	     {name:'toStatus',index:'toStatus', width:80, align:"center", sorttype:"int"},
        	        	     {name:'on', index:'on',width:130, align:"center", sorttype:"int"},
        	        	     {name:'to', index:'to', width:115, align:"center", sorttype:"int"},
        	        	     {name:'onH', index:'onH', width:120, align:"center", sorttype:"int"},
        	        	     {name:'fromLocation',index:'fromLocation', width:60,align:"center", sorttype:"int"},
        	        	     {name:'toLocation', index:'toLocation',width:120, align:"center"},
        	        	     {name:'reason', index:'reason',width:80, align:"center"},
        	        	     {name:'fromExpirationDate', index:'fromExpirationDate',width:80, align:"center"},
        	        	     {name:'toExpirationDate', index:'toExpirationDate',width:80, align:"center"},
        	        	     {name:'lastModifiedDate', index:'lastModifiedDate',width:80, align:"center"}],
 	      ...
 	      jQuery("#transactionHistory").jqGrid('navGrid','#pager',{view:true,add:false,edit:false,del:false, searchtext:'Filter'},{},{},{},{multipleSearch:true});
 	     jQuery("#transactionHistory").jqGrid('hideCol', 'cb');

 	    $("form#transactionForm").submit(function() {
 		    var newUrl = "/cpsb/transactionHistory.do?method=getTransactionHistoryDetails&" + $(this).serialize();
 		    $("#transactionHistory").jqGrid("setGridParam",{"url": newUrl, datatype:"json"}).trigger("reloadGrid");
 		    return false;
 		}); 

        $("select#tdate").change(function(){
         
            if(($("#tdate option:selected").text())=== 'Between' )
            {
                $("input#edate").show();
                $("input#edate").val('');
                $("input#sdate").val('');
                $("img[src$='icon_cal2.png']").show(); 
                $("#edate").attr('name', 'endDate');
               
            }
            
            else if((($("#tdate option:selected").text())=== 'Select period')||(($("#tdate option:selected").text())=== 'Equal')
                    ||(($("#tdate option:selected").text())=== 'Before')||(($("#tdate option:selected").text())=== 'After'))
            {
                $("input#sdate").val('');
                $("input#edate").val('');
                $("input#edate").hide();
                $("img[src$='icon_cal2.png']").hide(); 
                $("#edate").removeAttr('name');
